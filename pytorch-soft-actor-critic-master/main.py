...
import itertools
import torch
from sac import SAC  # SAC算法的自定义实现
...
parser.add_argument('--eval', type=bool, default=True, help='Evaluates a policy every 10 episodes (default: True)')
...
for i_episode in itertools.count(1):
    ...
    while not done:
        if args.start_steps > total_numsteps:
            ...
            action = agent.select_action(state)  # 从策略中采样动作
        ...
        next_state, reward, done, _ = env.step(action)  # 步进
        episode_steps += 1
        total_numsteps += 1
        episode_reward += reward

        # 忽略"done"信号，如果是由于到达时间限制引起的。
        mask = 1 if episode_steps == env._max_episode_steps else float(not done)

        memory.push(state, action, reward, next_state, mask)  # 将转换附加到内存中
